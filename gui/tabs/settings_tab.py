...
class SettingsTab(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.error_delay_input = QtWidgets.QSpinBox()
        self.error_delay_input.setRange(0, 5000)
        self.error_delay_input.setSingleStep(100)
        self.error_delay_input.setSuffix(" ms")
        
        training_layout.addRow("電腦走棋延遲:", self.delay_input)
        training_layout.addRow("錯誤顯示延遲:", self.error_delay_input)
    ...
    def load_settings(self, user_data: dict):
        ...
        self.error_delay_input.setValue(user_data.get("error_display_delay_ms", 1000))

    def _on_save(self):
        settings = {
            "lichess_username": self.lichess_user_input.text().strip(),
            "training_delay_ms": self.delay_input.value(),
            "error_display_delay_ms": self.error_delay_input.value()
        }
